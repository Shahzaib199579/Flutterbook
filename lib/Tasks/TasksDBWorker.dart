...
class TasksDBWorker {
  ...
  Future<Database> init() async {
    ...
    Database db = await openDatabase(
      ...
      version: 1,
      onOpen: (db) {},
      onCreate: (Database inDatabase, int intVersion) async {
        await inDatabase.execute("CREATE TABLE IF NOT EXISTS tasks("
  ...
  Future<List> getAll() async {
    ...
    var list = rec.isNotEmpty ? rec.map((m) => taskFromMap(m)).toList() : [];
